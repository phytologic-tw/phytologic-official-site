    ...
    <main style={page}>
      <nav style={nav}>
        ...
        <div style={links}>
          ...
          <a style={link} href="#contact">合作</a>
        </div>
      </nav>
          ...
          {colors.map((c, i) => (
            <div
              ...
              style={{
                ...orb,
                background: c[3],
                transform: `rotate(${i * 60}deg) translate(210px) rotate(-${i * 60}deg)`,
              }}
            />
          ))}
          <div style={centerOrb}>
            <div style={{ fontSize: 22, opacity: 0.78 }}>PLANT × AI</div>
            <div style={{ fontSize: 58, fontWeight: 950 }}>PHYTOLOGIC</div>
          </div>
        </div>

        <div style={heroCopy}>
          <div style={tag}>AI HEALTH SYSTEM・植物機能飲</div>
          <h1 style={h1}>
            讓每一個人
            <br />
            活得久，
          ...
          <p style={lead}>
            植本邏輯不是在做飲料，而是用植物營養、身體邏輯與派森 AI，
            建立現代人每天都能執行的健康修復系統。
          </p>
          <div style={btns}>
            <a href="#ai" style={primary}>開始 AI 健康分析</a>
            <a href="#colors" style={ghost}>探索六色系統</a>
          </div>
        </div>
      </section>

      <section style={blackBand}>
        <div style={bigText}>
          文明進步了，<br />身體卻越來越疲憊。
        </div>
        <div style={chips}>
          {["熬夜", "外食", "久坐", "螢幕", "高壓", "慢性發炎"].map((x) => (
            <span style={chip} key={x}>{x}</span>
          ))}
        </div>
      </section>

      <section id="story" style={story}>
        <div style={small}>BRAND STORY</div>
        <h2 style={h2}>50歲成為父親後，我第一次認真思考：</h2>
        <div style={quote}>「我還有多久能陪他？」</div>
        <p style={text}>
          這不是一個從商業開始的品牌，而是一位父親、一位丈夫、六個家庭重新理解健康後的答案。
          我們不是做飲料，我們做的是每天會進入身體、長期被吸收、影響未來十年與二十年的食物。
        </p>
      </section>

      <section id="colors" style={colorsSection}>
        ...
        <div style={colorList}>
          {colors.map((c) => (
            <div style={colorRow} key={c[0]}>
              <div style={{ ...colorDot, background: c[3] }} />
              <div style={{ flex: 1 }}>
      ...
      <section id="contact" style={contact}>
        ...
        <p style={{ ...text, color: "rgba(255,255,255,.78)", margin: "0 auto", maxWidth: 860 }}>
...
const brandEn = { fontSize: 25, fontWeight: 950, letterSpacing: "-1px" };
...
const hero = {
  ...
  background:
    "radial-gradient(circle at 20% 20%, rgba(34,197,94,.18), transparent 28%), radial-gradient(circle at 80% 70%, rgba(250,204,21,.16), transparent 30%)",
};
...
const orbWrap = {
  ...
  boxShadow: "0 0 120px rgba(34,197,94,.14)",
};
...
const bigText = { fontSize: 58, lineHeight: 1.1, fontWeight: 950, letterSpacing: "-2px" };
const chips = { display: "flex", flexWrap: "wrap", gap: 14 };
const chip = {
  padding: "18px 26px",
  borderRadius: 999,
  background: "#07130d",
...
const colorsSection = { padding: "96px 72px", background: "#0b1c12" };
...
const colorDot = { width: 76, height: 76, borderRadius: "50%", flexShrink: 0 };
...
const contact = {
  margin: "40px 72px 70px",
  padding: "80px 60px",
  borderRadius: 44,
  textAlign: "center",
  background: "linear-gradient(135deg, #14532d, #031008)",
};
